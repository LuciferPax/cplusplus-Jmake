...
def jmake(json_file='jmake.json'):
    ...
    with open(json_file, 'r') as f:
        ...
        for include_dir in data['include_dirs']:
            ...
            for file in os.listdir(include_dir):
                if file.endswith('.cpp'):
                    ...
                    # Compile source file
                    subprocess.run(['g++', '-c', file_path, '-o', output_file_path] + include_dirs)
        ...
        subprocess.run(['g++', '-c', main_file, '-o', main_output_file_path] + include_dirs)
        linkable_files.append(main_output_file_path)

        # Link all objects
        subprocess.run(['g++', '-o', os.path.join(output_dir, 'main')] + linkable_files)
